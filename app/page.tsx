...
function CountUp({ value, decimals = 0, suffix = '', duration = 1.8 }: { value: number, decimals?: number, suffix?: string, duration?: number }) { const [count, setCount] = useState(0); const [started, setStarted] = useState(false); useEffect(() => { if (!started) return; let start = 0; const t0 = performance.now(); const frame = (now: number) => { const p = Math.min((now - t0) / (duration * 1000), 1); const eased = 1 - Math.pow(1 - p, 3); setCount(value * eased); if (p < 1) requestAnimationFrame(frame); else setCount(value); }; requestAnimationFrame(frame); }, [started, value, duration]); return <motion.span className="stat-number" onViewportEnter={() => setStarted(true)} viewport={{ once: true, amount: .8 }}>{count.toFixed(decimals)}{suffix}</motion.span> }
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="stats"><div className="container stats-grid"><Reveal><div><b><CountUp value={5} decimals={1} /></b><span>★★★★★</span><small>Note patients</small></div></Reveal><Reveal delay={.08}><div><b><CountUp value={115} suffix="+" /></b><span>AVIS</span><small>Expériences partagées</small></div></Reveal><Reveal delay={.16}><div><b><CountUp value={2} /></b><span>DOCTEURS</span><small>Une équipe complémentaire</small></div></Reveal><Reveal delay={.24}><div><b><CountUp value={6} /></b><span>SPÉCIALITÉS</span><small>Une prise en charge complète</small></div></Reveal></div></section>
